...
class Manage extends Backend {
    ...
    public function index() {
        ...
        $manager_data = new ArrayObject($this->kb_auth_model->get_all_managers($filter), ArrayObject::ARRAY_AS_PROPS);
    ...
    public function users() {
        $user_list = new ArrayObject($this->kb_auth_model->get_all_users(), ArrayObject::ARRAY_AS_PROPS);
    ...
    public function add($entreprise_label = '') {
        ...
        if ($this->form_validation->run('add_manager') !== FALSE) {
            ...
            if ($this->kb_auth->add($username, $password, $email, $user_data, $type = 2)) {
                // Flash success
