...
class Bank_statements extends Entreprises {
    ...
    function __construct() {
        ...
        $this->template->set_partial('sidebar', 'tpl/bank_statements/sidebar', $this->data);
    ...
    public function add() {
        ...
        if ($this->form_validation->run('bank_statements') !== FALSE) {
            $data = array(
                'bank_account_id' => underscore($this->input->post('bank_account_id')),
                'label' => underscore($this->input->post('label')),
                'description' => $this->input->post('description'),
                'date' => strtotime($this->input->post('date')),
            ...
            if ($this->accounting_model->add_new_bank_statement($data)) {
                $this->session->set_flashdata('success', lang('success_message'));
            } else {
                $this->session->set_flashdata('error', lang('error_message'));
            }

            foreach ($this->config->item('bank_statements') as $validation) {
        ...
        $this->template->build('tpl/bank_statements/create_view', $this->data);
    }
    ...
    function upload_process() {
        ...
        if (!$this->upload->do_upload()) {
            ...
            $this->session->set_userdata('file_path', $data['upload_data']['file_name']);
            $this->session->set_userdata('raw_name', $data['upload_data']['raw_name']);
        }
    }
